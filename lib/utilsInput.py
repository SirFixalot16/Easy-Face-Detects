...
def augment_data(images):
    ...
    for i in range(0,images.shape[0]):
        ...
        if np.random.random() > 0.75:
            images[i] = tf.keras.preprocessing.image.random_rotation(images[i], 20, row_axis=0, col_axis=1, channel_axis=2)
        if np.random.random() > 0.75:
            images[i] = tf.keras.preprocessing.image.random_shear(images[i], 0.2, row_axis=0, col_axis=1, channel_axis=2)
        if np.random.random() > 0.75:
            images[i] = tf.keras.preprocessing.image.random_shift(images[i], 0.2, 0.2, row_axis=0, col_axis=1, channel_axis=2)
        if np.random.random() > 0.75:
...
def data_generator_pose(X,Y,batch_size):
    ...
    while True:
        ...
        for i in range(len(X)):
            ...
            if len(p) == batch_size:
                yield augment_data(np.array(p)), np.array(q)
                #return augment_data(np.array(p)), np.array(q)
                p,q = [],[]
        if p:
            yield augment_data(np.array(p)), np.array(q)
            #return augment_data(np.array(p)), np.array(q)
