...
from keras.layers import Conv2D, AveragePooling2D, MaxPooling2D, BatchNormalization, SeparableConv2D, Reshape
...
class SSR_net:
    def __init__(self, image_size,stage_num,lambda_local,lambda_d):
        # Set data format to shape (size, size, color channel) instead of (color channel, size, size)
        ...
        if K.image_data_format() == "channels_first":
            ...
            # (color channel, size, size)
        else:
            logging.debug("image_dim_ordering = 'tf'")
            self._channel_axis = -1
            self._input_shape = (image_size, image_size, 3)
            # (size, size, color channel)
...
#    def create_model(self):
    def __call__(self):
        ...
        # May run into issues when importing/loading model because of this Lambda layer
